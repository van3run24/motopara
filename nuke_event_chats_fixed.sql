-- ИСПРАВЛЕННЫЙ ЯДЕРНЫЙ СКРИПТ (правильный синтаксис PostgreSQL)
-- Полное уничтожение всех ссылок на event_chats
...
BEGIN
    FOR view_record IN 
        ...
        AND view_definition LIKE '%event_chats%'
    LOOP
...
UNION ALL  
SELECT 'triggers_on_event_chats' as object_name,
       (SELECT COUNT(*) FROM information_schema.triggers WHERE trigger_schema = 'public' AND event_object_table = 'event_chats') as status;
