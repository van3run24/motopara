-- Исправляем RLS политики для group_chat_participants чтобы getUserChats работал
DROP POLICY IF EXISTS "Users can view own group chat participants" ON group_chat_participants;
...
-- Более гибкая политика для просмотра
...
-- Также убедимся что group_chats имеет правильные политики
DROP POLICY IF EXISTS "Users can view own group chats" ON group_chats;
