-- Создание таблиц для групповых чатов событий
...
-- Триггер для автоматического создания чата при создании события
CREATE TRIGGER on_event_create_chat
    AFTER INSERT ON events
    FOR EACH ROW
    EXECUTE FUNCTION create_event_chat();

-- Добавляем поле max_participants в таблицу events если его нет
DO $$
BEGIN
